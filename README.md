...
Codex Quota Float 是一个轻量的 Windows 桌面小组件：紧凑状态显示共享 Codex 本周总额度的剩余百分比，单击后展开查看周额度与重置时间。它依赖用户另行安装的 Win-CodexBar，不是 OpenAI、Codex 或 Win-CodexBar 的官方项目，也未获得这些项目的背书。

## 功能

- 60 × 60 像素置顶悬浮球，显示共享 Codex 本周总额度与环形进度。
- 单击展开 250 像素宽的详情卡；只显示 1 周总额度，不再显示已取消的 5 小时额度。
- 按 7 天窗口时长识别周额度，兼容周额度出现在 CLI `primary` 或 `secondary` 字段中的情况。
- 额外模型额度保持独立，不会被合并或误标为共享 Codex 总额度。
- 显示剩余百分比、重置时间、套餐标签和数据新鲜度状态。
- 每 60 秒尝试刷新；刷新失败时保留最近一次成功缓存，并标记为数据暂未更新。
- 支持拖动定位、记住位置、立即刷新、打开 Win-CodexBar 完整统计、开机启动和单实例运行。
- 失去置顶状态时自动恢复，也可从右键菜单手动“重新置顶”。
- 按剩余额度切换蓝、黄、红三档提示色。

## 界面

当前版本以共享 Codex 周总额度为唯一主指标：紧凑态显示周剩余百分比；单击悬浮球后，窗口展开为周额度详情卡，显示套餐、本周剩余、进度条和重置时间。

### 周额度展开态

![周额度展开态：显示套餐、本周剩余、进度条和重置时间](assets/widget-weekly-expanded.png)
...
把下面整段提示词交给 Windows 上的 Codex：

```text
请在这台 Windows 电脑上从零安装 Codex Quota Float 周额度版。只使用项目仓库 https://github.com/zsy1122334455-netizen/codex-quota-float 的最新 Release，并先读 README；若 Release 尚未包含周额度版，停止并告诉我。
若未安装 Win-CodexBar，用 winget 安装 Finesssee.Win-CodexBar；下载 Release 的 ZIP 与 SHA-256，校验后解压并运行 install.ps1。
安装后运行 codexbar-cli.exe usage -p codex --format json；退出码为 0 但 JSON 含 error 或无有效 usage 仍算失败，需要登录时让我操作。
若 CLI 受代理影响，只在单次 CLI 进程测试现有 HTTP_PROXY/HTTPS_PROXY；成功后备份启动器，仅让悬浮球继承代理，禁止修改全局代理。
只有显示真实周百分比、可单击展开并正常刷新才算成功。联网、安装、执行或修复前，先说明来源、用途和影响。
```
...
- 本地数据位于 `%APPDATA%\CodexQuotaFloat`：`last_usage.json` 保存处理后的共享周额度、重置描述和上游更新时间；`settings.json` 保存窗口位置。它们是当前 Windows 用户可访问的普通 JSON 文件。
